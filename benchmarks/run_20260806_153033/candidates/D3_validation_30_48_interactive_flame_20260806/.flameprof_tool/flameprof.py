...
def prepare(funcs, calls, threshold=0.0001):
    ...
    def _calc_back(names, level, to, origin, visited, pw):
        if to and names:
                    total = sum(calls[(r, to)][3] for r in names)
                    # cProfile can report a zero-duration edge for a C/CUDA
                    # boundary even when its parent has wall time.  Keep that
                    # edge at zero width rather than aborting SVG rendering.
                    factor = pw / total if total > 0 else 0
        else:
            factor = 1
