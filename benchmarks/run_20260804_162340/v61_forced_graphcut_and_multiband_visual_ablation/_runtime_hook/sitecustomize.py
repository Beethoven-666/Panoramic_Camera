"""Process-local, unsafe GraphCut+MultiBand visual ablation only."""
...
def _apply_forced_multiband(old_bgr, new_bgr, owner_bgr, owner_new, eligible, guards, **_kwargs):
    # This is intentionally unsafe: it removes the guard exclusion only for
    # the blender call, while leaving GraphCut labels and provenance unchanged.
    unguarded = replace(guards, protected=np.zeros_like(guards.protected, dtype=bool))
    return _apply(
        old_bgr, new_bgr, owner_bgr, owner_new, eligible, unguarded,
...
renderer.apply_near_multiband = _apply_forced_multiband
