...
def _sample_process(
    ...
    try:
        ...
        target.update(
            {"peak_process_tree_rss_bytes": peak, "rss_sample_count": samples}
        )
...
def _sdk_worker(arguments: list[str]) -> int:
    ...
    if operation == "frozen-2d":
        ...
        sdk.run_post_3d(
            payload["session"], payload["two_d_output"], output_dir=payload["output"]
        )
    else:
...
def _run_one(
    ...
    with (
        ...
        process = subprocess.Popen(
            command, stdout=stdout, stderr=stderr, text=True, env=os.environ.copy()
        )
...
def main() -> int:
    ...
    (root / "doctor.json").write_text(
        json.dumps(Gemini305VideoSDK().doctor().as_dict(), indent=2)
    )
    samples = []
    ...
    summary = {
        **identity,
        "status": "PASS" if success == args.runs else "FAIL",
