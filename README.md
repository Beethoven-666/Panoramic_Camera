...
主扫描段的全部真实优化 pose nodes 都是源（至少 2、最多 160），不再压缩成 32 张全画布投影。每个源的原始中央带不超过 RGB 宽度的 `20%`；如果 hard-owner 区不能放入该窄带，会要求更密的真实采样或更低输出尺度，而不会扩大条带。条带临时落盘，输出阶段只加载相邻 2 条（配置硬上限 5）；画布和流式 aggregate working set 均不超过 `200 MP`。
