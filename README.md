...
默认配置会明确开启彩色自动曝光。自动曝光改善暗场亮度，但可能在大棚暗处延长曝光并引入运动模糊；正式以 1.5 m/s 运行前，应检查 `frames.csv` 的 `color_exposure` 和实际拖影。若需固定短曝光，可使用 `--exposure-us 800` 切回手动模式；800 µs 期间小车移动约 1.2 mm，对应约 1.6 个原始图像像素。

Gemini 305 的彩色曝光控制原始单位是 100 µs；采集器会把 `--exposure-us` 的微秒值换算后再写设备。`frames.csv` 的 `color_exposure` 保留设备原始值，例如 `301` 表示约 30.1 ms，而不是 301 µs。
...
默认配置已经开启自动曝光，也可以显式使用 `--auto-exposure`。还可通过命令行覆盖 `--warmup-frames`、`--queue-size`、`--gain` 和 `--white-balance`。长期运行建议复制 `configs/demo.yaml` 后修改自定义配置，不要直接改变公共默认值。
...
真实相机以 30 fps、最高 1.5 m/s 采集时，可先使用配置中的默认 `stride: 3`：
...
- `--stride` 和 `--max-frames` 覆盖配置中的关键帧选择；
...
- `--blend-mode scan_seam` 为侧扫默认值：沿扫描轴给每帧分配单源区域，只在相邻区域的有限重叠带内用 `COST_COLOR_GRAD` 图割搜索颜色与梯度代价较低的接缝，再执行多频段融合。低代价不等于保证绕开所有前景；GraphCut 失败会明确报错，不会回退到重叠平均。旧的 `feather` 会平均所有重叠帧，只保留用于对照；
- `--render-frame-ids` 可从完整输入中指定少量清晰原帧用于最终成图，它们的平移由已验证的布局轨迹插值得到；
- `--translation-anchor-y` 控制从完整单应矩阵提取平移的位置，近景侧扫可取 `0.85`，避免空白背景主导前景布局；
- `--scan-seam-margin` 与 `--scan-multiband-levels` 分别控制接缝搜索宽度和接缝低频平滑层数；
...
- `--scan-protect-region FRAME_ID:X0:Y0:X1:Y1` 可重复使用，让指定清晰帧完整拥有未裁剪画布中的近景矩形。它用于禁止 GraphCut 穿过存在明显视差的完整物体，坐标和 owner 都会写入报告；这是输入相关的高级覆盖项，不应照搬到不同路线；
...
5. 将通过验证的变换投影到配置的运动模型；默认 `translation` 会在配置的纵向锚点提取平移，使布局匹配真正关心的扫描深度层；
6. 将每条受约束的 `source_to_reference` 变换累积到第一帧坐标系；
7. `scan_seam` 从原始全分辨率帧中选择清晰关键帧，先补偿全局曝光，再以扫描轴分配单源区域，在相邻重叠带中寻找颜色/梯度低代价接缝；可选的近景保护矩形会在 GraphCut 后强制完整物体由单帧拥有。最终只对 owner 边界做窄 mask 软化与多频段融合，并按有效 mask 的最大矩形自动裁剪 `panorama.jpg`；GraphCut 失败会使本次渲染失败，而不是退回重叠平均。
...
└─ pairs/             默认保存的逐对 UniStitch FFD/TPS 预览
   ├─ 0000_0001.jpg
   └─ ...
```
...
- `translation_anchor_y: 0.85`：在画面下部近景层提取平移；
- `sequence_blend_mode: scan_seam`：禁止几十张重叠帧整幅平均；
- `scan_max_keyframes`、`scan_seam_margin`、`scan_multiband_levels`：分别控制清晰源帧上限、相邻区域接缝搜索带宽和多频段融合层数；源帧数也会计入 scan aggregate working-set；
...
- `max_canvas_megapixels: 250`：限制单个最终几何画布；在 `scan_seam` 下也限制“画布 MP × 最终渲染源帧数”的 aggregate working-set；
- `save_pair_previews: true`：默认保留逐对诊断图。
...
- **行程长度**：`max_frames: 40` 是布局阶段的 Demo 上限；`max_canvas_megapixels: 250` 同时约束单画布，并在 `scan_seam` 下约束“画布 MP × 最终渲染源帧数”的 aggregate working-set。长路线应按约 2–5 m 分块，并只维护子地图级全局关系。
...
先检查运动模糊、曝光、相邻帧重叠和图片尺寸。真实 30 fps 数据可把 `--stride` 从 3 降至 2 或 1。默认模式同时验证 UniStitch global 与 LightGlue + MAGSAC；MAGSAC 必须满足最低内点数和至少 50% 的内点率，并且择优比较会让两种变换使用同一组 MAGSAC 内点。使用 `--strict-unistitch` 会禁用 MAGSAC 的择优与回退路径。周期铁架场景不要通过盲目放宽误差阈值来强行接受错误边。
