...
├─ configs/capture_unrestricted_auto_exposure.yaml  无安全曝光上限的诊断采集覆盖
...
诊断模式仍逐帧记录 `color_exposure`，但不会因超过 `800 µs` 连续三帧而停止。若相机不支持设置和回读 AE 最大范围，或回读值仍未高于被替换的正式上限，命令会明确失败，避免把仍受旧上限约束的数据误标为“无限制”。
...
无限制自动曝光采集得到的会话必须使用此选项；不传时命令会在模型加载前拒绝，并写入 `failure.json`：
