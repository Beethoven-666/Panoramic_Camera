...
RGB 会话或合成会话
  ...
  └─ unistitch-sequence
       ├─ UniStitch global homography 或可选 MAGSAC 恢复
...
| 彩色曝光 | `800 µs` | 建议在 `500–1000 µs` 内配合补光调节 |
...
`800 µs` 曝光期间小车移动约 1.2 mm，对应约 1.6 个原始图像像素；若边缘仍模糊，应优先增加无频闪漫射补光并缩短曝光。不要单纯提高帧率来代替短曝光。
...
还可通过命令行覆盖 `--warmup-frames`、`--queue-size`、`--gain` 和 `--white-balance`。长期运行建议复制 `configs/demo.yaml` 后修改自定义配置，不要直接改变公共默认值。
...
- `--strict-unistitch` 禁止 MAGSAC 布局恢复：UniStitch global 分支未通过重投影验证时直接失败。
...
`pair_unistitch.jpg` 使用推理分辨率生成，不是原始全分辨率计量图。`pair_report.json` 同时保存 UniStitch 原始 global homography 和最终采用的 `homography_source_to_reference`；`layout_method` 为 `unistitch_global` 或 `magsac_fallback`。即使最终全局布局回退到 MAGSAC，双图预览仍由完整 UniStitch FFD/TPS 产生。
...
- `--max-canvas-megapixels` 在分配全局画布前执行硬限制；
...
- `--strict-unistitch` 禁止 LightGlue + MAGSAC 恢复，任一 UniStitch global 边不合格即终止；
...
3. global 分支通过时记录 `unistitch_global`，否则在默认配置下允许使用 LightGlue 内点经 USAC/MAGSAC 得到 `magsac_fallback`；
4. 将通过验证的变换投影到配置的运动模型；默认 `translation` 会从 UniStitch global 变换的内部采样点稳健提取平移，抑制固定侧扫中微小投影误差的累计放大；
5. 将每条受约束的 `source_to_reference` 变换累积到第一帧坐标系；
6. 完成全部布局后，从原始全分辨率帧一次性透视变换和羽化，生成 `panorama.jpg`。
...
- **行程长度**：`max_frames: 40` 和 250 MP 画布是 Demo 安全阈值。长路线应按约 2–5 m 分块，并只维护子地图级全局关系。
...
先检查运动模糊、曝光、相邻帧重叠和图片尺寸。真实 30 fps 数据可把 `--stride` 从 3 降至 2 或 1。默认模式会在 UniStitch global 分支不合格时尝试 LightGlue + MAGSAC；使用了 `--strict-unistitch` 则不会恢复。周期铁架场景不要通过盲目放宽误差阈值来强行接受错误边。
