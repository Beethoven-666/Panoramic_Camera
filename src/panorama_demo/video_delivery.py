...
def invalidate_video_delivery(output: Path) -> None:
    ...
    for name in ("video_delivery.json", "video_report.json", "video_failure.json", "video_panorama.jpg", "video_panorama.png", "video_pixel_provenance.npz"):
...
def publish_video_2d(output: Path, panorama: np.ndarray, owner: np.ndarray, report: dict[str, Any]) -> dict[str, Any]:
