...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="development-only post-P2 fixture directory for same-parent M6 comparison",
    )
    parser.add_argument(
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if s13_spec is not None:
        ...
        report = run_s13_experiment(
            ...
            post_p2_fixture=getattr(args, "post_p2_fixture", None),
        )
        return report
    if (
        run_offline_orb
        or ignore_pose
        or simulate_optimizer_all_fail
        or getattr(args, "m62_execution_mode", None)
        or getattr(args, "post_p2_fixture", None)
    ):
