...
严格 RGB-D 会话、标定、曝光、清晰度与主扫描段
  ...
  → Open3D 相邻 RGB-D odometry
  → 仅由 RGB-D 边构建并优化 pose graph
  → 有限 4×4 camera_to_world SE(3)
  → 原始全分辨率 RGB-D 正射深度重投影侧扫条带
  → 深度硬约束的相邻 pair OpenCV GraphCut
  → 严格单 owner 拓扑
  → 每条相邻 owner 边界独立局部 OpenCV MultiBand
  → 裁剪、质量门禁和原子交付
```

正式路径不得导入或回退到 UniStitch、LightGlue、MAGSAC、Torch、3×3 单应矩阵、二维累计或时间/二维运动位姿插值。`unistitch-pair` 暂时保留为可选历史双图诊断；`unistitch-sequence` 只是 `g305-panorama` 的弃用别名，运行同一 RGB-D 路径。
...
| `rgbd_odometry.py` | Open3D 延迟导入、RGB-D odometry、边质量、pose graph、SE(3) 与轨迹审计 |
...
- pose graph 只能包含 RGB-D odometry 边；相邻边必需，非相邻弱边最多按配置验证真实重叠，不得用特征匹配补边。
...
## 6. GraphCut、owner 与 MultiBand

正式 backend 固定为 `graphcut_depth_constrained`。OpenCV GraphCut 本身不接收深度代价，深度约束必须先转成硬 masks：
