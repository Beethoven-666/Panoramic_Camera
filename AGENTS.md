...
正式全景输出像素只能取自 RGB。正式渲染路径不得读取 aligned depth 来生成像素、拟合参考平面、以深度产生前景，亦不得导入或回退到 UniStitch、LightGlue、MAGSAC、Torch、3×3 单应矩阵、二维累计或时间/二维运动位姿插值。完成 RGB-only 全景质量门禁后，`g305-panorama` 额外构建仅供浏览的 TSDF 网格并发布 `tsdf_mesh.glb` 与 `tsdf_mesh_viewer.html`；该产物只读取严格 RGB-D 会话和已验证真实轨迹，绝不得向全景的条带、接缝、融合、裁剪或质量判定回传任何结果。RGB-D 深度保留给严格会话契约、Open3D/ORB-SLAM3 的真实轨迹验证及这个独立展示产物。ORB-SLAM3 仅提供真实 RGB-D 相机轨迹；未安装或未完整跟踪时应失败，不能回退为伪造位姿。`unistitch-pair` 暂时保留为可选历史双图诊断；`unistitch-sequence` 只是 `g305-panorama` 的弃用别名，运行同一 RGB-D 路径。`central_strip_plane_diagnostic` 只能由独立诊断命令通过 renderer callback 调用，绝不能成为正式 RGB pushbroom backend、正式 CLI 选项或失败回退。
...
默认正式 backend 为 `calibrated_rgb_pushbroom`。风险仅由相邻 RGB 的 Lab 残差与梯度残差检测；亮度比例仅在共同有效、低梯度、低残差、未过曝/欠曝的白墙候选上以 trimmed median/Huber 估计，并在帧序列上平滑增益曲线。不得把无效去畸变黑边当作纹理。`graphcut_depth_constrained` 仅保留给历史回归；TSDF 仅可在 RGB 全景门禁后用于独立 3D 展示，二者绝不能作为正式渲染或失败回退。
...
2026-07-16 已通过 `D:\Panoramic_Camera\.conda\Scripts\g305-panorama.exe` 的 editable 主入口复验同一会话，输出 `outputs/greenhouse_sequence_optimized/`：101 个真实源，`2978×782`，裁剪高度 `97.75%`、融合区 `1.264%`、融合风险/owner 边界风险/受保护组件拆分均为 `0`、峰值驻留条带 `2`、亮度 gain `0.755–1.364`，并发布 `delivery.json`。交付声明 `projection=calibrated_rgb_pushbroom`、`seam_backend=rgb_monotonic_hard_owner_graphcut`；当时该目录未输出 TSDF mesh 或深度前景产物。这是当前直接 CLI 入口的实机验收，仍不代表所有场景或速度都必然成功。
...
- [ ] RGB 全景路径无 UniStitch/Torch/LightGlue/MAGSAC、TSDF、RGB-D projection 或 central-strip plane renderer 导入/回退；TSDF 仅在其后作为独立展示导出；
