...
正式 `handoff_fallback_policy` 必须显式保持 `publish_degraded=true`、`local_apap_flow_enabled=false`、`manual_review_for_grade_c=true`。它不是放宽结构安全门限，而是将**已完整审计且结构安全**的视觉结果与严格质量结果分开表达：

- **A**：全部严格质量门通过，所有相邻 handoff 都是安全 anchor/owner；发布 `delivery_state=published`。
- **B**：全部严格质量门通过，至少一个相邻 handoff 使用已审计的既有局部逆网格（报告为 `flow_mesh`），或已显式启用且已完整审计的 `local_apap_flow`；仍发布 `delivery_state=published`。默认关闭时，既有局部逆网格仍可形成 B；只有实际 audit 标明 APAP/flow 方法时才可声称使用了该候选。
- **C**：会话、物理连续的真实轨迹、一次 inverse remap、owner 拓扑与每对 handoff 审计均完整，但低 fitness/RMSE、输入/最终图像等严格质量未通过，或完整有效走廊采用已审计的最低代价 hard cut。它发布 `delivery_state=published_degraded`、`quality_grade=C`，并强制 `manual_review_required=true`；仍必须发布只读 TSDF GLB 与 Viewer，且不以它们或平均/补洞掩盖问题。
- **F**：标定、aligned depth/单位/元数据、真实轨迹、有限 SE(3)、必需边/图连通、有效 RGB remap、owner/GraphCut/MultiBand 拓扑、资源硬限或原子交付任一结构项失败。物理不连续、逆向、步长/扫描跨度、垂直或前后漂移、旋转或 RGB-D 边残差越界同样属于真实轨迹 F。F 不发布全景或 `delivery.json`，只保留原子 `failure.json`。

正式 `report.json` 与 `delivery.json` 使用 `/v9`，都包含 `delivery_state`、`strict_quality_pass`、`quality_grade`、`handoff_fallback_summary`、`foreground_owner_continuity_summary`、`tsdf_visualization` 和 `manual_review_required`；`handoff_fallback_summary` 固定计数 `anchor`、`apap`、`flow_mesh`、`hard_cut`。报告还保留逐 pair 的 `handoff_outcomes` 与严格失败原因。`quality_pass` 仅保留为 v8 兼容的严格质量别名，不能单独判断是否已经发布；应结合最后原子写入的 `delivery.json` 与 `delivery_state`。
...
| 路径 | 职责 |
|---|---|
| `configs/demo.yaml` | 正式零调参采集、RGB-D 位姿验证、RGB pushbroom、接缝/融合安全默认值，以及 A/B/C 发布的 `handoff_fallback_policy` |
...
| `stitch_sequence.py` | 严格 RGB-D 位姿编排 + 正式 RGB pushbroom、A/B/C/F 发布判定、v9 报告、失败清理和原子交付 |
...
- 优化结果必须有限、旋转正交、行列式为 +1、图连通，并保持连续单向侧移；缺失必需边、非有限/非刚体 SE(3)、不连通、物理不连续/逆向、步长或扫描跨度异常、垂直/前后漂移、旋转或 RGB-D 边残差越界、以及无法审计的残差始终是 F。只有在这些真实轨迹结构条件完整后，低 fitness/RMSE 等可审计质量不足才可通过 A/B/C 中的 `strict_quality_pass` 和严格失败原因透明交付。
- 正式渲染必须保留主扫描段的全部真实优化 pose nodes（至少两帧、最多 160 帧）；不得抽稀成 32 个全画布源、插值、重排或伪造中间位姿。若相邻真实相机中心按已审计 RGB 像素/毫米尺度换算后不足独立 owner 所需的 `0.25 px`，允许仅抑制该近重复节点的独立 owner 区间：真实 pose、轨迹边和一次全分辨率 remap 全部保留，最终颜色只由其前后保留真实节点的已 remap RGB hard owner 覆盖，不得混合、变形、生成颜色或插值 pose/gain；覆盖或审计不完整仍为 F，完整时透明降为 C。
- 每个源只进行一次全分辨率、标定 inverse remap；近重复抑制节点的临时条带也必须实际 remap 后再审计为零最终 owner。其它中间源只保留其主点附近的窄条；首帧和末帧仅向扫描方向外侧扩展至各自校准 RGB 图像边缘，以保留端点场景。端点扩展仍须由单一 hard owner 和独立 valid mask 支撑；最终裁剪若丢弃完整有效的端点外扩列则失败，畸变校正导致的几何无效边缘列须审计后裁去。临时条带落盘，任一阶段内只保留相邻 2 个 RGB 条带（配置硬上限 5）。
...
- `local_apap_flow` 的候选边界比既有逆网格更窄：仅一个相邻 `96–160 px` 走廊、仅双向可见的同层安全背景、非 protected RGB 像素、仅一个 hard owner 的 inverse sampling；当前前景实例仍不得解除 owner-only。即使其局部节点拟合内部使用带权 DLT，也绝不是全局/pose/全景级 `3×3` 变换，不得进入 `transforms.json`、条带布局或真实轨迹。正式默认关闭；只有显式启用后，40+ 对应、inlier、held-out、前后向 flow、`16/32 px` 网格、边界零位移、`≤8 px` 位移、正 Jacobian、`0.80–1.25` 尺度及保护域零交集全部通过时才可使用。任一失败只给出 `hard_cut_degraded` 建议，不能半信任地应用局部 warp。
- 条带的 x 坐标按真实相机中心的单调 SE(3) 位移，并以相邻 RGB 局部运动/毫米的稳健标量换算；该标量只决定条带布局，绝不是二维位姿、单应矩阵或深度/平面代理。尺度不稳定时失败；只有尺度本身可靠而个别真实节点换算间距 `≤0.25 px` 时，才可进入上述近重复 owner 抑制。
...
- 全景输出只包含 RGB、独立 valid mask 和 hard-owner 审计；不得产生 `surface_depth_mm`、相机深度或前景 mask。A/B/C 交付目录必须包含只读展示用的 `tsdf_mesh.glb` 和 `tsdf_mesh_viewer.html`；任何网格都不得影响全景。
- 画布与流式 aggregate working set 均不得超过 `200 MP`；诊断也不能提高这些硬限。pose nodes 最多 160。

## 6. RGB 风险、hard owner 与窄带融合

默认正式 backend 为 `calibrated_rgb_pushbroom`。RGB Lab/梯度风险始终先约束 owner/MultiBand；只有满足上条跨 seam 中线的结构性 raw seed，或边缘残差/整高 hard cut 明确指出几何问题时，才读取深度辅助。未触发时沿用纯 RGB 接缝。触发后只有已验证同层能得到局部逆网格，近景、把手、软管、遮挡、深度孔洞、透明/反光内容均为单一 RGB hard owner，不能进入 MultiBand。透明斜带等原始 RGB 内容不是“待修复的错位”，应保留其单源外观。既有已审计局部逆网格的相邻 pair 可记为 B 级 `flow_mesh`；已审计、完整有效的 hard cut 可记为 C 级，而不是偷偷平均。`local_apap_flow` 默认关闭；即使启用也必须有逐 pair 完整 audit，不能仅凭配置声称正式路径使用了 APAP/稠密 flow。亮度比例仅在共同有效、低梯度、低残差、未过曝/欠曝的白墙候选上以 trimmed median/Huber 估计，并在帧序列上平滑增益曲线。不得把无效去畸变黑边当作纹理。`graphcut_depth_constrained` 仅保留给历史回归；TSDF 仅可在 A/B/C 全景交付后用于独立 3D 展示，二者绝不能作为正式渲染或失败回退。

- 仅相邻扫描源在互斥 pair corridor 内竞争；
- 风险包括 Lab/梯度残差与其膨胀保护带；高风险像素及保护带只允许一个 RGB owner，绝不进入 MultiBand；
- 若组件级 owner 无法找到不穿过保护带的物理安全边界，只有一幅 RGB 条带完整覆盖有效走廊时才允许整走廊单源；此回退适用于 geometry 和纯 RGB 组件拓扑，不能因非 geometry pair 直接失败。若最终 valid mask、单一 owner、相邻 pair 审计和 RGB 覆盖仍完整，则该最低代价 hard cut 是 C 级人工复核交付；若没有完整覆盖或 owner 拓扑不完整，仍为 F 且 fail-closed。任何因这一决定不再贡献颜色的真实节点必须在审计中列出，所有真实 pose 仍参与轨迹、布局和一次 remap；
- GraphCut 在每个相邻条带重叠区寻找单调 hard owner；无法找到安全通道时允许报告经审计的 hard cut 并降为 C，绝不以透明重影掩盖；GraphCut/MultiBand 结构错误本身仍是 F；
...
正式最终门限包括：融合区风险严格为 `0`、融合区占有效像素不超过 `20%`、曝光增益 `0.45–2.20`、裁剪高度至少 `85%`、画布宽度至少 `95%`。这些是 `strict_quality_pass` 的组成；在会话、真实轨迹、remap、owner 拓扑和 handoff 审计仍完整时，严格质量未过会透明地降为 C，而不是将其伪装为 F。
...
正式输出使用 `gemini305-calibrated-rgb-pushbroom/v10` 与 `gemini305-panorama-delivery/v10`。默认 `source_aware_global_linear_rgb_v2_two_stage` 先优先验证中性 safe-wall，其不足时仅以固定 16×16、80/20 train/held-out 审计通过的同表面非中性 RGB 纹理建立相邻 gain 关系；可靠 pair 参与部分观测全局增益求解，之后重新计算亮度/色差风险，只有原始结构风险继续受保护。触发 RGB-D 风险时只可复用既有 96–160 px 走廊中的双向可见同层背景，不能为光度校正增加深度读取。缺 pair、交叉方法冲突或 held-out 失败不得伪造观测；失败 pair 仍以 `identity_hard_owner` 透明降级，但已经验证的其它 pair 不再被清零。失败 pair 只有在 post-gain 后存在双向可见同层、非保护、低梯度且色差一致的背景时，才允许在 96 px 搜索走廊内使用 2–8 px 局部 MultiBand；其余区域仍为单调 hard owner。owner 分区与有效覆盖完整时发布 C，并逐 pair 记录失败原因；无法形成完整单调 owner 分区仍为 F。已验证光度观测非有限、gain 越界或病态求解仍是 F。两份文件都必须公开 `delivery_state`、`strict_quality_pass`、`quality_grade`、`handoff_fallback_summary`、`photometric_calibration`、`foreground_owner_continuity_summary`、`tsdf_visualization` 和 `manual_review_required`；报告另含逐 pair `handoff_outcomes`、严格失败原因和嵌套 `publication` 审计。`photometric_calibration` 不影响 handoff 计数：其 `rgb_texture_consistent` 会使严格质量通过的安全结果成为 B，所有 pair 都为 `safe_wall` 时才可为 A；`identity_hard_owner` 对应的 handoff 必须计为 C 级 `hard_cut`。`quality_pass` 是严格质量的兼容别名，**不是**“是否已发布”的唯一判断：A/B 使用 `published`，结构安全但需人工复核的 C 使用 `published_degraded`。C 级仍是 RGB-only 全景，必须标明人工复核并交付成对的 TSDF GLB/Viewer；诊断产物不采用这些正式交付语义。`g305-foreground-deformation-diagnostic` 成功时同样严格只写 `diagnostic_panorama.jpg` 与 `diagnostic_report.json`；其默认模式检查一个 pair，显式 `--whole-panorama` 可在完整链中逐 pair 审计并合成诊断全景。报告使用独立 schema，包含 `foreground_deformation_audits`、候选/拒绝原因和 held-out 标量，绝不发布 `delivery.json`。
...
`outputs/greenhouse_sequence_optimized/` 是 2026-07-16 留存的历史 CLI 输出：101 个真实源，`2978×782`，裁剪高度 `97.75%`、融合区 `1.264%`、融合风险/owner 边界风险/受保护组件拆分均为 `0`、峰值驻留条带 `2`、亮度 gain `0.755–1.364`。其 JSON sidecar 是早于当前 geometry-assist 审计的 `/v2`，不含本版本要求的深度辅助网格和 A/B/C handoff 字段；只能证明当时的旧路径/真实轨迹，不得当作当前 `/v7`、`/v9`、`/v9` schema 的正式验收。当前 Windows 主机恢复 WSL/ORB-SLAM3 后须重新执行 CLI 才能发布新 `delivery.json`。
...
- [ ] RGB 全景路径无 UniStitch/Torch/LightGlue/MAGSAC、TSDF、RGB-D projection 整幅投影或 central-strip plane renderer 导入/回退；禁止全局/pose/全景级 `3×3`，仅允许受审计的相邻 RGB-D 局部可见性/逆网格；`local_apap_flow` 也只能在已显式启用的同层 `96–160 px` 局部走廊内使用；TSDF 在 A/B/C 全景判定后作为独立、必需的展示导出；
- [ ] 失败路径先失效旧 delivery 并写 `failure.json`；
- [ ] A/B/C 成功路径最后发布 v9 `delivery.json`，并核对 `delivery_state`、`strict_quality_pass`、`quality_grade`、`handoff_fallback_summary` 与 `manual_review_required`；F 只写 `failure.json`；
